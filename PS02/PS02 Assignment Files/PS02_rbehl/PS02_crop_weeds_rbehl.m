...
% ENGR 132 
% Program Description 
% This program is meant to use the given data to help the engineer anylaze
% and break the data into field pixels and then use field pixels under many
% different condications.
...
%Part E
field_pixels_visualinspection = find(weeddata(:,:) > 0.95); %finding the field pixels that have weed percentage greater than 95%
newfield_pixels_visualinspection = weeddata(field_pixels_visualinspection); % creating a new martix with only the data for the visualinspection
[Row,Col] = find(newfield_pixels_visualinspection);
...
fprintf('The pixel locations that require visual inspection are %.f, %.f\n',[Row,Col]);
